...
TEST_CASE_METHOD(TApp, "NoEscape1", "[escape]") {
    ...
    std::string str;
    app.add_option("-s,--string,string", str)->transform(CLI::EscapeTransformer{});
...
TEST_CASE_METHOD(TApp, "Numbers", "[escape]") {
    ...
    std::string str;
    app.add_option("--number", str)->transform(CLI::EscapeTransformer{});
    // Example command line: `exe \\\-s` or `exe "\\-s"`
...
TEST_CASE_METHOD(TApp, "Flags", "[escape]") {
    ...
    std::string flags;
    std::string name;
    app.add_option("flags", flags, "flags")->transform(CLI::EscapeTransformer{});
